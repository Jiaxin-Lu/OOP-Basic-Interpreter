...
struct LINE
{
  ...
  string str = "";
  LINE(int num, string strr) : line_number(num), str(strr), state(nullptr), ne(nullptr){};
};
 ...
 */
  ...
  ***New one!
  * -----------------------------------------------
  * determine whether this lineNumber occured before.
  * Also determine whether this lineNumber is smaller than FIRSTLINE.
  */
  bool usedLineNumber(int lineNumber, bool &beforeFirst);
 ...
 */
  ...
  void RUN_(EvalState &state);
  void LIST_();
  void CLEAR_(Program &program, EvalState &state);
  void HELP_();
